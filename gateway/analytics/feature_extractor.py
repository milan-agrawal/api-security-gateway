...
def extract_features(events: List[SecurityEvent]) -> dict:
    ...
    if len(timestamps) > 1:
        inter_arrivals = [(timestamps[1]-timestamps[i-1].total_seconds()) for  i in range(1, len(timestamps))]
    ...
    trottled_ratio = throttled/total_requests
    ...
    return {
        ...
        "throttled_ratio": round(trottled_ratio, 3),
    }